...
from ptsr.basemodel import *


class PatternWeightNet(nn.Module):
    def __init__(self, args, input_dim, output_dim, num_layers, hidden_dim=512):
...
class PTSR(Basemodel):
    def __init__(self, args, writer) -> None:
        super(PTSR, self).__init__(args)
        self.args = args
        self.writer = writer
    ...
    def distance_to_weight(self, pattern_target_dis, mask):
        ...
        weight = F.softmax(pattern_target_dis, dim=-1)  # (B, Y, X)
        return weight 
    ...
    def accumulate_pattern_kl_bias_weight(self, seq, pos, neg):
        ...
        pos_weight = pos_weight + both_weight.unsqueeze(1) * self.mlp_lambda  # (B, 1, N)
    ...
    def forward(self, seq, pos, neg):         
        pos_score, neg_score, pos_weight = self.accumulate_pattern_kl_bias_weight(seq, pos, neg)  # (B, 1) (B, Y)
        return pos_score, neg_score, pos_weight
    ...
    def case_study(self, seq, pos, neg):
        pos_score, neg_score, pos_weight = self.accumulate_pattern_kl_bias_weight(seq, pos, neg)  # (B, 1)  (B, Y)  (B, 1, N)
        scores = torch.cat((pos_score, neg_score), dim=-1)  # (B, 1 + Y)
        return scores, pos_weight